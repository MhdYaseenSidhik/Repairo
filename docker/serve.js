...
// Serves files from its own directory. "/" lists the designs/ HTML previews so
// the container's primary purpose (previewing the designs) is one click away.
...
// Build the "/" landing page: links to every HTML preview under designs/.
function designIndex() {
  ...
  const items = files.length
    ? files
        .map((f) => `<li><a href="/designs/${encodeURIComponent(f)}">${f}</a></li>`)
        .join("")
    : "<li>No HTML previews under designs/ yet.</li>";
  return (
    "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\">" +
    "<title>Repairo — design previews</title></head><body>" +
    "<h1>Repairo design previews</h1><ul>" +
    items +
    "</ul></body></html>"
  );
}
...
const server = http.createServer((req, res) => {
  ...
  // path that escapes the site directory (directory-traversal guard).
  let urlPath;
  try {
